...
class MultiFileReader:
    ...
    def __init__(self):
        self._open_files = {}
        # self.client = storage.Client()
        # self.bucket = self.client.bucket('314968306_index')

    def read(self, locs, n_bytes, base_dir):
        b = []
        for f_name, offset in locs:
            if f_name not in self._open_files:
                #blob = self.bucket.get_blob(f'anchor_index/{f_name}')
                #self._open_files[f_name] = blob.open('rb')
                self._open_files[f_name] = open(Path(base_dir + '/' + f_name), 'rb')
    ...
    def posting_lists_iter(self, base_dir):
        ...
        with closing(MultiFileReader()) as reader:
            for w, locs in self.posting_locs.items():
                b = reader.read(locs[0], self.df[w] * TUPLE_SIZE, base_dir)
                posting_list = []
